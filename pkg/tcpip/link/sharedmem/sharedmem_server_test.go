...
func newClientStack(t *testing.T, qPair *sharedmem.QueuePair, peerFD int) (*stack.Stack, error) {
	ep, err := sharedmem.New(defaultMTU, defaultBufferSize, localLinkAddr, qPair.TXQueueConfig(), qPair.RXQueueConfig(), peerFD, nil)
	if err != nil {
...
func newServerStack(t *testing.T, qPair *sharedmem.QueuePair, peerFD int) (*stack.Stack, error) {
	ep, err := sharedmem.NewServerEndpoint(defaultMTU, defaultBufferSize, localLinkAddr, qPair.TXQueueConfig(), qPair.RXQueueConfig(), peerFD, nil)
	if err != nil {
		return nil, fmt.Errorf("failed to create sharedmem endpoint: %s", err)
	}
...
func TestServerRoundTrip(t *testing.T) {
	...
	dialFunc := func(address, protocol string) (net.Conn, error) {
		conn, err := gonet.DialTCP(ctx.clientStk, listenAddr, ipv4.ProtocolNumber)
		if err != nil {
			return nil, err
		}
		return conn, nil
