...
func parseToken(token string) (start, end uint32, run bool, err error) {
	ts := strings.SplitN(token, "-", 2)
	switch len(ts) {
	case 0:
		return 0, 0, false, fmt.Errorf("invalid token %q", token)
	case 1:
		val, err := strconv.ParseUint(ts[0], 10, 32)
		if err != nil {
			return 0, 0, false, err
		}
		return uint32(val), uint32(val), false, nil
	case 2:
		...
		return uint32(val1), uint32(val2), true, nil
	default:
...
func parseBitmap(input string, sizeHint uint32) (*bitmap.Bitmap, error) {
	...
	for _, t := range tokens {
		start, end, run, err := parseToken(strings.TrimSpace(t))
		if err != nil {
			return nil, err
		}
